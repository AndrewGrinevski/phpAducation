...
if (isset($_GET['id']) && !empty($_GET['id']) && $_GET['action'] == 'edit_product') {
    ...
    $ids_category = $page['category_id'];
    $stmt_category = mysqli_prepare($connection, "SELECT * from shop_category WHERE id = ?");
    mysqli_stmt_bind_param($stmt_category, "d", $ids_category);
    mysqli_stmt_execute($stmt_category);
    $res_category = mysqli_stmt_get_result($stmt_category);
    $category_types = mysqli_fetch_assoc($res_category);

    $ids_sub_category = $page['sub_category_id'];
    $stmt_sub_category = mysqli_prepare($connection, "SELECT * from shop_category WHERE id = ?");
    mysqli_stmt_bind_param($stmt_sub_category, "d", $ids_sub_category);
    mysqli_stmt_execute($stmt_sub_category);
    $res_sub_category = mysqli_stmt_get_result($stmt_sub_category);
    $category_types = mysqli_fetch_assoc($res_sub_category);
                            ...
                            <form class="form-horizontal form-bordered" role="form" method="post"
                                ...
                                <div class="form-group">
                                    ...
                                    <div class="col-sm-8">
                                        <select name="category" class="form-control">
                                            <?php foreach ($categories as $category):
                                                $selected = '';
                                                if (isset($page['sub_category_id']) && $page['sub_category_id'] == $category['id']) {
                                                    $selected = 'selected';
                                                }
                                                ?>
                                                <option value="<?= $category['id'] ?>" <?= $selected ?> > <?= $category['name'] ?></option>
                                ...
                                <?php if ($_GET['action'] == 'add_product') { ?>
                                    <div class="form-group">
                                        ...
                                        <div class="col-sm-8">
                                            <input type="text" placeholder="Default Input" name="new_category"
                                                   value="<?= $category_types['name'] ?? '' ?>"
                                                   class="form-control"/>
