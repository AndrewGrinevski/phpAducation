...
if (!empty($_POST['name']) && !empty($_POST['password'])) {
    ...
    if ($user) {
        if ($user['password'] == md5($_POST['password'])) {
            $_SESSION['user'] = $user;
            var_dump($_SESSION);
            header("Location: http://homestead2.test/admin");
        }
    }
}

if (!empty($_POST['fullname']) && !empty($_POST['email']) && !empty($_POST['rpassword']) && !empty($_POST['confirmpassword'])) {
    $fullname = $_POST['fullname'];
    $email = $_POST['email'];
    $password = md5($_POST['rpassword']);

    $stmt = mysqli_prepare($connection, "INSERT INTO users (`username`, `email`, `password`)
VALUES (?,?,?)");
    ...
    <div class="frame">
        ...
        <div ng-app ng-init="checked = false">
            <form class="form-signin" action="" method="post" name="form">
                ...
                <input class="form-styling" type="text" name="name" placeholder="" autocomplete="off"/>
                <label for="password">Password</label>
                <input class="form-styling" type="password" name="password" placeholder="" autocomplete="off"/>
            ...
            <form class="form-signup" action="" method="post" name="form">
                ...
                <label for="password">Password</label>
                <input class="form-styling" type="text" name="rpassword" placeholder=""/>
